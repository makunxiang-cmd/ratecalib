...
> - **soft 版 raking/logit 已实现（2026-06-21）**：惩罚校准——边际硬约束，目标行加 ridge
>   `Omega^{-1}=size^2/(2 lambda grand_total priority)`（与 chi2 soft 语义一致）。对偶 Newton 加按约束的
>   `reg` 向量：`F=Ax-t+reg*lambda`，`J += diag(reg)`（正定）。`test-distance.R` 覆盖（边际硬保持、
>   恒正/界内、lambda 越大越贴近、不可达也有解）。
> - **统一 `A x = t` 抽象层：已决定不做（2026-06-21）。** 约束装配（`M`/`R`/`rate_rhs`）其实**已在两条
>   求解路径间共享**（构造一次，chi2 的 OSQP 与 raking/logit 的对偶 Newton 都复用）；不共享的只有求解器
>   本身，而它们本就该不同（QP+箱约束 vs 牛顿）。强行让 chi2 也走对偶 Newton 会丢掉 OSQP 的箱约束处理，
>   属有风险的无价值重构。故不做。
...
>
> **更新（2026-06-21）：均值/总量 soft 模式与 `proportions=` 便捷接口均已实现。** mean/total soft 的惩罚按
> 目标量级归一化（相对误差），解决率 vs 数值的尺度不可比；§二 至此全部完成。
...
## 五、进度反馈（进度条）· P2 · ✅ **已实现（重复循环处）**

> **实现状态（2026-06-21）**：按本节结论，进度条落在**外层重复循环**——
> `calibrate_replicate_weights(..., progress = TRUE)` 用 `utils::txtProgressBar`（base，无新依赖）。
> 单次求解仍不做进度条（聚合后变量少、求解快，收益低）。
...
## 七、方差 / 不确定性估计 · P2 · 🟡 **部分实现（重复权重已落地）**

> **实现状态（2026-06-21）**：**重复权重路线已落地**（`package/R/replicate.R`）：
> `calibrate_replicate_weights(fit, repweights, scale, rscales, progress)` 对每套重复权重以 `fit` 的同一目标/
> 设置重新校准（目标自 `fit$target_check` 重建），`replicate_variance(object, x, statistic)` 用
> `Var = scale·Σ rscales_r·(θ̂_r−θ̂_0)²` 估计 total/mean 的方差/SE，与 `survey::svrepdesign` 对齐。
> `progress=TRUE` 用 `utils::txtProgressBar` 在重复循环显示进度（即 §五 的落点）。`test-replicate.R` 覆盖
> （exact 目标量方差≈0、非目标量方差>0、方差随 scale 线性、相同权重方差=0、行数/正性校验、进度条）。
>
> **⛔ design-based 线性化方差（路线 1）：已决定不实现（2026-06-21）。** 理由：Deville–Särndal 线性化方差
> 要求把初始权重当作设计权重 `d=1/π`（故 `π=1/d≤1`，即 `d≥1`），并需抽样设计信息（分层/聚类/包含概率）。
> 本包把权重当作**通用正权重**——演示数据即含 `<1` 的权重（`π>1` 非法），且不建模分层/聚类。强行套
> with-replacement 公式会对常见权重设置给出**错误甚至负的方差**，比不提供更糟。**重复权重路线（上方，已实现）
> 是设计正确的替代**：用户把真实抽样设计编码进重复权重即可。若将来要做线性化方差，须先引入显式的抽样设计
> 抽象（strata/PSU/fpc/π），那是另一个包级别的设计决策。
...
2. ✅ **距离族（§一）** — **已完成**：raking 熵距离与 logit 有界距离（exact + soft，对偶 Newton）作为
   opt-in 落地，默认仍 chi2（非破坏性）。**已决定不做**：默认改 raking（破坏性）、统一 `A x = t` 抽象层
   （约束装配已共享，强行统一无价值且有风险）——见 §一 顶部。
3. ✅ **目标统计量泛化：比例/均值/总量（§二）** — **已全部完成**：均值/总量（充分统计量，exact+soft）+
   任意分类变量占比（按指示变量拆纯单元）+ `make_rate_targets(means=/totals=/proportions=)`。
...
6. ✅ **方差估计 + 重复权重（§七）+ 进度条（§五）** — **重复权重 + 进度条已实现**。**已决定不做**：
   design-based 线性化方差（要求权重为设计权重 1/π≥1 且需抽样设计信息，与本包通用权重定位冲突；重复权重
   是设计正确的替代）——见 §七 顶部。
