...
> - **尚未实现**：把 chi2/raking/logit 统一到 `A x = t` 抽象层
>   （现 chi2 仍走独立 OSQP 分支）。
...
> **尚未实现**：design-based 线性化方差（路线 1）——需引入本包尚不建模的抽样设计信息（包含概率/分层），
> 单独立项。
...
6. 🟡 **方差估计 + 重复权重（§七）+ 进度条（§五）** — **重复权重 + 进度条已实现**；**仅余** design-based
   线性化方差（需抽样设计信息抽象，单独立项）。
