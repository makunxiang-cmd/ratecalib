...
> - **默认仍 `"chi2"`**，旧行为逐字节保留；raking 为 opt-in，**未跳版本号**。
> - raking 当前**仅 `mode="exact"`**；soft 版 raking（惩罚校准）、**logit 距离**、以及**把默认改成 raking**
>   （破坏性，需跳 minor 版本 + NEWS 标注）均**尚未实现**，留作后续子任务。
> - raking 上方无界，`lower`/`upper` 对它不强制，仅在诊断中报告越界倍数（与下文"安全阀"设计一致）。
...
## 二、目标统计量泛化：**比例 / 均值 / 总量**（务必区分，处理非 0/1 数据）· P1
...
## 六、交互目标（cross-classification）· P2
...
- 与 §二 正交：交互目标同样可是 proportion / mean / total。
...
2. 🟡 **约束装配抽象 + 距离族，默认 raking（§一）** — **部分实现**：raking 熵距离（exact，对偶 Newton）
   已作为 opt-in 落地（默认仍 chi2，非破坏性）；**剩余**：soft 版 raking、logit 距离、把默认改成 raking
   （破坏性，需跳版本号）、以及把 chi2/raking 统一到 `A x = t` 抽象层。
3. **目标统计量泛化：比例/均值/总量（§二）** — 解决非 0/1 真实数据；复用 §一 统一层，重构聚合为充分统计量。
4. ~~**Excel 输入输出（§四）**~~ ✅ **已实现**（`read/export` + `calibrate_from_excel()`，openxlsx 走 Suggests）。
5. **交互目标（§六）** — 在统一目标行机制上增量。
