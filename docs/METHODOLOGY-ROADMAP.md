...
## 一、距离函数族（Deville–Särndal 校准族）— 旗舰扩展 · P1 · **默认改为熵/raking** · 🟡 **部分实现**
...
> - **尚未实现**：soft 版 raking/logit（惩罚校准）、**把默认改成 raking**（破坏性，需跳 minor 版本 +
>   NEWS 标注）、以及把 chi2/raking/logit 统一到 `A x = t` 抽象层（现 chi2 仍走独立 OSQP 分支）。
...
## 二、目标统计量泛化：**比例 / 均值 / 总量**（务必区分，处理非 0/1 数据）· P1 · 🟡 **部分实现**
...
>
> **⚠️ 对下文设计的关键修正**：下文「聚合改造」建议按分组单元聚合、每单元存内部比例 `ā_c` 且**不按取值
> 拆单元**——这对 proportion **不成立**：现有 0/1 合格率正是靠**按 outcome 拆纯单元**才能完全控制组内率；
> 若改成存 `ā_c` 不拆，组率只能在各单元 `ā_c` 凸包内移动，**控制力被削弱、破坏向后兼容**。故：
> **mean/total 用充分统计量（连续变量本就无法拆纯单元，凸包控制是标准做法，已实现）；
> 「任意分类变量的占比」必须按指示变量拆单元，需重构聚合层，尚未实现**（替代：把 `I(Z==v)`
> 预编码成 outcome 走现有路径）。
...
2. 🟡 **约束装配抽象 + 距离族，默认 raking（§一）** — **部分实现**：raking 熵距离与 logit 有界距离
   ...
   把默认改成 raking（破坏性，需跳版本号）、以及把 chi2/raking/logit 统一到 `A x = t` 抽象层。
3. 🟡 **目标统计量泛化：比例/均值/总量（§二）** — **部分实现**：均值/总量（充分统计量，exact，chi2/raking/logit）
   + `make_rate_targets(means=/totals=)` 已落地；**剩余**：任意分类变量的占比（需按指示变量拆单元、重构聚合层）、
   以及 mean/total 的 soft 模式。
...
> 凡涉及默认/接口变更（尤其 §一 默认改 raking、§二 目标表扩列），同步更新 `NEWS.md` 与版本号。
