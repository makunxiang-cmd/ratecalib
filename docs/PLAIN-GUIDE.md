...
> **分组变量用数字也行。** 分组列既可以是文字（`Urban`/`F`），也可以是数字编码（如学历段 `1,2,3,4`）
> 或因子——程序内部会统一当作"类别"处理。唯一要注意：在目标表里写类别时按**字符串**对应，
> 比如数据里的 `1` 对应目标里的 `"1"`（用 R 的命名向量 `c("1"=0.72, "2"=0.68, ...)` 即可，自动满足）。
> 但分组应是**离散类别**，别拿连续小数（如连续年龄、收入）当分组——那种需求请用"均值/总量"目标（见 11.3）。
> 注意：这只针对分组列；**"是否合格"列仍必须是 0/1**。
...
| 分组变量是数字编码（1/2/3/4）能用吗 | 内部按"类别"处理，数字/文字/因子都行 | 目标类别按字符串写，如 `c("1"=0.72, "2"=0.68)`；分组须离散，连续值改用均值/总量目标 |
